...
def compute_scores(task_name, federated_dataset, sensitive_attribute, sensitive_attribute_type, split,
    trainers_dict, lmra_trainers_dict, criterion, is_binary_classification, learning_rate, optimizer_name,
    ...
    for attacked_client_id in tqdm(range(num_clients)):
        ...
        for client in trainers_dict.keys():
            if client != "global":
                ...
                scores_per_client_dict["lmra"][client] = 0
        ...
        try:
            lmra_model = lmra_trainers_dict[f"{attacked_client_id}"].model
        except KeyError:
            lmra_model = lmra_trainers_dict[attacked_client_id].model
        ...
        lmra_model_loss, lmra_model_metric = evaluate_trainer(lmra_trainers_dict[f"{attacked_client_id}"], dataloader)
        ...
        logging.info(f"Local model {split} loss={local_model_loss:.3f}, Test {split} metric={local_model_metric:.3f}")
        logging.info(f"LMRA model {split} loss={lmra_model_loss:.3f}, Test {split} metric={lmra_model_metric:.3f}")
        logging.info(f"Random model {split} loss={random_model_loss:.3f}, Test {split} metric={random_model_metric:.3f}")

        metrics_dict["local"][attacked_client_id] = local_model_metric
        metrics_dict["lmra"][attacked_client_id] = lmra_model_metric
        ...
        aia_score_lmra = evaluate_aia(model=lmra_model, dataset=dataset, num_iterations=aia_num_rounds,
            sensitive_attribute_id=sensitive_attribute_id, sensitive_attribute_type=sensitive_attribute_type,
        ...
        logging.info(f"Score={aia_score_lmra:.3f} for client {attacked_client_id} with lmra model")
        ...
        scores_per_client_dict["lmra"][attacked_client_id] = aia_score_lmra
...
def main():
    ...
    with open(args.lmra_models_metadata_path, "r") as f:
        lmra_models_metadata_dict = json.load(f)
    ...
    lmra_trainers_dict = initialize_trainers_dict(
        lmra_models_metadata_dict, criterion=criterion, model_init_fn=model_init_fn,
        is_binary_classification=is_binary_classification, metric=metric, device=args.device
    )
    ...
    scores_per_client_dict, metrics_dict, n_samples_list = compute_scores(
        ...
        lmra_trainers_dict=lmra_trainers_dict,
    ...
    global_scores = list(metrics_dict["local"].values())
