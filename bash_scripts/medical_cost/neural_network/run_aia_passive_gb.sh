...
base_cmd="python run_aia.py \
  ...
  --num_rounds 100 \
  --device $device \
  --logs_dir ./logs/seeds/$seed/medical_cost/$split_criterion/$n_tasks/$batch_size/local_epochs/$local_epochs/$optimizer/aia_${lr}_${frac} \
  --log_freq 1 \
  --seed $seed \
...
keep_rounds_frac=(0.20 )  # Add keep_rounds_frac values
...
for lr in "${learning_rates[@]}"; do
  for attr in "${sensitive_attributes[@]}"; do
    for frac in "${keep_rounds_frac[@]}"; do
      ...
      --results_path ./results/seeds/$seed/medical_cost/$split_criterion/$n_tasks/$batch_size/local_epochs/$local_epochs/$optimizer/aia_${lr}_${frac}.json"
