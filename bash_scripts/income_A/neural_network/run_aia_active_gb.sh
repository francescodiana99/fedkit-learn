...
base_cmd="python run_aia.py \
  --task_name income \
  --temperature 1.0 \
  --metadata_path ./metadata/seeds/$seed/income/full/$n_tasks/$n_task_samples/$batch_size/$local_epochs/sgd/federated.json \
  ...
  --active_server \
  --local_models_metadata_path ./metadata/seeds/$seed/income/full/$n_tasks/$n_task_samples/$batch_size/$local_epochs/sgd/isolated_trajectories_${attacked_round}.json \
  --isolated "
...
for lr in "${learning_rates[@]}"; do
  for attr in "${sensitive_attributes[@]}"; do
    for frac in "${keep_rounds_frac[@]}"; do
      ...
      --results_path ./results/seeds/$seed/income/full/$n_tasks/$n_task_samples/$batch_size/$local_epochs/sgd/isolated/${attacked_round}/aia_${lr}_${frac}.json \
      --logs_dir ./logs/seeds/$seed/income/full/$n_tasks/$n_task_samples/$batch_size/$local_epochs/sgd/isolated/${attacked_round}/aia_${lr}_${frac}"
