...
for lr in "${learning_rates[@]}"; do
  for attr in "${sensitive_attributes[@]}"; do
    for frac in "${keep_rounds_frac[@]}"; do
      ...
      --results_path ./results/seeds/$seed/income/$state/mixed/$mix_percentage/$n_tasks/$batch_size/$local_epochs/$optimizer/aia_${lr}_${frac}.json \
      --logs_dir ./logs/seeds/$seed/income/$state/mixed/$mix_percentage/$n_tasks/$batch_size/$local_epochs/$optimizer/aia_${lr}_${frac} "
