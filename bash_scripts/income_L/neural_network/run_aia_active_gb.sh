...
base_cmd="python run_aia.py \
  --task_name income \
  --temperature 1.0 \
  --metadata_path ./metadata/seeds/$seed/income/$state/mixed/$mix_percentage/10/$batch_size/$local_epochs/$optimizer/federated.json \
  --data_dir ./data/seeds/$seed/income/tasks/correlation/$state/$mix_percentage/10/all \
  --local_models_metadata_path ./metadata/seeds/$seed/income/$state/mixed/$mix_percentage/10/$batch_size/$local_epochs/$optimizer/isolated_trajectories_${active_round}.json \
  --split train  \
  --optimizer sgd \
  --num_rounds 100 \
  --device $device \
...
for lr in "${learning_rates[@]}"; do
  for attr in "${sensitive_attributes[@]}"; do
    for frac in "${keep_rounds_frac[@]}"; do
      ...
      --results_path ./results/seeds/$seed/income/$state/mixed/$mix_percentage/$n_tasks/$batch_size/$local_epochs/$optimizer/isolated/${active_round}/aia_${lr}_${frac}.json \
      --logs_dir ./logs/seeds/$seed/income/$state/mixed/$mix_percentage/$n_tasks/$batch_size/$local_epochs/$optimizer/isolated/${active_round}/aia_${lr}_${frac}.json "
